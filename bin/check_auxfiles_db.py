...
def draw_png(SoundID, item_flac, item_wav, sampling_rate, max_freq_draw, spectrogram_palette):
	...
	#Large size
	status, output = commands.getstatusoutput('./svt.py -s ' + item_flac[:-5] + '-large_s.png -w 870 -h 400 -m ' + max_freq_draw + ' -f 4096 -p ' + str(spectrogram_palette) + ' ' + item_wav)
	if status != 0:
		print " "
		print "There was a problem processing " + item_flac + "!"
		print output
...
def draw_png_stereo(SoundID, item_flac, item_wav, sampling_rate, max_freq_draw, spectrogram_palette):
	...
	#Large size
	#left
	status, output = commands.getstatusoutput('./svt.py -c 1 -s ' + str(SoundID) + '-large_s.png -w 870 -h 400 -m ' + max_freq_draw + ' -f 4096 -p ' + str(spectrogram_palette) + ' ' + item_wav)
	if status != 0:
		print " "
		print "There was a problem processing " + item_flac + "!"
		print output
